...
export default function AccountingReports() {
  ...
  const handleDownload = (apiCall: Promise<unknown>, filename: string) => {
    void downloadBlob(apiCall, filename, (e: unknown) => {
      toast({ variant: "destructive", title: t("accountingPages.downloadFailed"), description: (e as Error)?.message });
    });
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon="AR" title={t("accountingPages.reportsTitle")} subtitle={t("accountingPages.reportsSubtitle")} />

      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <label htmlFor="accounting-report-month" className="text-sm text-muted-foreground">{t("months.month")}</label>
        ...
        <label htmlFor="accounting-report-year" className="text-sm text-muted-foreground">{t("months.year")}</label>
        ...
        <BrandButton variant="secondary" size="sm" onClick={setCurrentPeriod}>{t("accountingPages.currentPeriod")}</BrandButton>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <BrandCard className="hover:shadow-hover cursor-pointer">
          <div className="flex flex-col items-center text-center gap-3 py-4">
            ...
            <h3 className="font-display text-base font-bold text-ink-dark">{t("accountingPages.trialBalance")}</h3>
            ...
            <BrandButton variant="primary" onClick={() => handleDownload(Accounting.trialBalancePdf(reportParams), "trial-balance.pdf")}>
              <DownloadIcon /> {t("financePages.generate")}
            </BrandButton>
          </div>
        </BrandCard>
        ...
        <BrandCard className="hover:shadow-hover cursor-pointer">
          <div className="flex flex-col items-center text-center gap-3 py-4">
            ...
            <h3 className="font-display text-base font-bold text-ink-dark">{t("accountingPages.incomeStatement")}</h3>
            <p className="text-sm text-muted-foreground">{t("accountingPages.incomeStatementHint")}</p>
        ...
        <BrandCard className="hover:shadow-hover cursor-pointer">
          <div className="flex flex-col items-center text-center gap-3 py-4">
            ...
            <h3 className="font-display text-base font-bold text-ink-dark">{t("accountingPages.balanceSheet")}</h3>
            <p className="text-sm text-muted-foreground">{t("accountingPages.balanceSheetHint")}</p>
      ...
      <DataTable
        title={t("accountingPages.trialBalancePreview")}
        compact
        columns={[
          { key: "account_code", label: t("accountingPages.code"), render: (v) => <span className="font-mono text-xs">{v}</span> },
          { key: "account_name", label: t("accountingPages.account"), sortable: true },
          { key: "total_debit", label: t("accountingPages.debit"), render: (v) => v ? renderCurrency(v) : <span className="text-muted-foreground/50">-</span>, align: "right" as const },
          { key: "total_credit", label: t("accountingPages.credit"), render: (v) => v ? renderCurrency(v) : <span className="text-muted-foreground/50">-</span>, align: "right" as const },
          { key: "balance", label: t("accountingPages.balance"), render: (v) => renderCurrency(v ?? 0), align: "right" as const },
        ]}
        data={trialRows}
        emptyMessage={t("accountingPages.noTrialBalance")}
      />
    </div>
  );
}
