...
class TokenIssuer
{
    ...
    public function pair(User $user, string $deviceName, ?string $family = null): array
    {
        ...
        $access->accessToken->forceFill(['token_family' => $family, 'device_name' => $deviceName])->save();

        $refresh = $user->createToken("refresh:$deviceName", ['refresh'], now()->addDays(30));
        $refresh->accessToken->forceFill(['token_family' => $family, 'device_name' => $deviceName])->save();
    ...
    public function limited(User $user, string $ability, string $deviceName, int $minutes): array
    {
        $family = (string) Str::uuid();
        $deviceName = Str::limit(trim($deviceName) ?: 'unknown-device', 255, '');
        $token = $user->createToken("$ability:$deviceName", [$ability], now()->addMinutes($minutes));
        $token->accessToken->forceFill(['token_family' => $family, 'device_name' => $deviceName])->save();
