...
| 14 | **Two sites round a computed third decimal away** — percentage adjustments (`StudentAdjustmentService.php:41`) and weighted-average stock costing (`ProcurementService.php:101`) | A 33.3333% scholarship loses 3 fils per invoice | Queued as its own order behind input-scale validation |
| 15 | **SQLite does not enforce decimal scale; PostgreSQL does** | Every financial test. Over-precise input round-trips intact in CI and is silently rounded in production | `decimal:0,2` validation on the seven money inputs that currently accept any scale — queued as the next order |
